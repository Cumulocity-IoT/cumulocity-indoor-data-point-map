...
const compile = series(
  ...
  function separateWebpackBuildSrc() {
    return fs.copy('./dist/widget-library/fesm5', './dist/bundle-src');
  },
...
const bundle = series(
  ...
  async function webpackBuild() {
    return execSync('npx webpack', { stdio: 'inherit' });
  },
  function copyCumulocityJson() {
    return fs.copy('./cumulocity.json', './dist/widget/cumulocity.json');
  },
  function createZip() {
    return (
      src('./dist/widget/**/*')
...
exports.default = series(clean, compile, bundle, async function success() {
  ...
  console.log(
    `Runtime Widget Output (Install in the browser): dist/${pkgJson.name}-${pkgJson.version}.zip`
  );
  console.log(
    `Widget Angular Library (Install with: "npm i <filename.tgz>"): dist/${pkgJson.name}-${
      pkgJson.version
    }.tgz`
  );
});
